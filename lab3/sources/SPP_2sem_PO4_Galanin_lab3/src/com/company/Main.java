...
public class Main {
    ...
    protected static void init_database() {
        ReferenceBook_Nomenclature.create_table(statement);
    }
    ...
    protected static void main_menu() {
        ...
        while(true) {
            ...
            if (key.equals("1")) {
                ReferenceBook_Nomenclature.create_interface(statement);
                press_any_key();
                continue;
            }

            if (key.equals("2")) {
                ReferenceBook_Nomenclature.read(statement);
                press_any_key();
                continue;
            }

            if (key.equals("3")) {
                ReferenceBook_Nomenclature.update_interface(statement);
                press_any_key();
                continue;
            }

            if (key.equals("4")) {
                ReferenceBook_Nomenclature.delete_interface(statement);
                press_any_key();
                continue;
            }
            ...
            if (key.equals("8")) {
                ReferenceBook_Nomenclature.drop_table(statement);
                press_any_key();
                continue;
            }
        }
